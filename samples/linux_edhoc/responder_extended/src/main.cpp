...
enum err process_ead_1(struct edhoc_responder_context *c, uint8_t *ead_1, uint32_t *ead_1_len)
{
	printf("processing ead 1!\n");
	PRINT_ARRAY("msg1 ead_1", ead_1, *ead_1_len);

	//test changing ead_2 in context c by application function
	uint8_t ead2_test[]={0x44,0x01,0x02,0x03,0x04};
 
	memcpy(c->ead_2.ptr,ead2_test,sizeof(ead2_test));
	c->ead_2.len=(uint32_t)sizeof(ead2_test);
...
int main()
{
	...
	c_r.ead_2.ptr = (uint8_t*)malloc(AD_DEFAULT_SIZE); //assign memory for ead_2
